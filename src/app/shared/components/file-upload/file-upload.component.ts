...
export class FileUploadComponent {
  ...
  @Input() acceptedFileTypes: string =
    'image/*,application/pdf,text/*,.doc,.docx,.xls,.xlsx';
  ...
  validateFile(file: File): boolean {
    if (file.size > this.maxFileSize) {
      this.alertService.handleError(
        `File ${file.name} is too large. Max size: ${this.formatFileSize(
          this.maxFileSize
        )}`
      );
      return false;
  ...
  async uploadFiles() {
    if (!this.bucketName || this.selectedFiles.length === 0) {
      this.alertService.handleError(
        'Please select files and enter a bucket name'
      );
      return;
    }
    ...
    try {
      ...
      for (const file of this.selectedFiles) {
        ...
        try {
          ...
          // Update progress
          const progressIndex = this.uploadProgress.findIndex(
      ...
      if (this.uploadedFiles.length > 0) {
        this.alertService.handleSuccess(
          `Successfully uploaded ${this.uploadedFiles.length} file(s) to Supabase!`
        );
